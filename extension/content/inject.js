...
(function () {
  ...
  // ---------- 解析纯函数委托 ----------
  // 纯解析逻辑抽到 parse.js（挂 window.__ttwParse），此处仅做解构绑定 + 运行时依赖注入，
  // 使同一套逻辑既服务 MAIN world 注入，又能被 Node 测试直接覆盖。
  const {
  ...
  setLog(log);
  setSfTrusted(() => _sfTrusted);
  setConvIdFromUrl(extractConvIdFromUrl);
  ...
  // 从 session_id 反推 created_at 由 parse.js 提供（deriveTimeFromSessionId）；
  // 启动时已 setSfTrusted(() => _sfTrusted) 注入自检开关。
