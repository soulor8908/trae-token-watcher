...
**trae-token-watcher** 是一个运行在浏览器里的「Token 账本」。它在你使用trae的官网 [www.trae.cn](https://www.trae.cn) 、论坛和traework 网页版时，**自动、无感地**记录每一次对话消耗的 Token、积分与费用，把分散在会话里的数据聚合成可量化、可回看、可优化的账本——**默认所有数据只存你本地浏览器 IndexedDB，不上传任何第三方**；若你在设置页主动开启「云端同步」，用量记录（含提问预览与来源 URL）会被加密备份到官方 Worker 以便跨设备查看，可随时关闭。
...
| **本地优先存储**  | 默认全部数据存浏览器 IndexedDB                                          | 默认不上传后端；开启云端同步后上传（见下文同步说明） |
...
1. **隐私优先** —— **默认**用量数据不上传项目方服务器（仅在主动开启「云端同步」后上传）；连诊断都可走「自有 Key 直连」零后端模式。
...
2. 粘贴你自己的 DeepSeek API Key，点保存（存入浏览器 `chrome.storage.local`，由当前系统用户 Profile 加密保护）；
3. 点「诊断我的 Token 用法」；
...
| 存储     | IndexedDB                    | 本地优先，默认不上传；开启云端同步后上传（见隐私说明）                       |
...
- **跨设备同步** —— 在用户明确授权下，支持可选的加密云端备份；备份内容为用量记录（含提问预览 `user_input_preview` 与来源 URL），不含对话正文，可随时在设置页关闭或清空
